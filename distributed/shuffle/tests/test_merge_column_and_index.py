...
async def test_merge_known_to_unknown(
    c, s, a, b, df_left, df_right, ddf_left, ddf_right_unknown, on, how, shuffle_method
):
    # Compute expected
    expected = df_left.merge(df_right, on=on, how=how)

    # Perform merge
    result_graph = ddf_left.merge(
...
async def test_merge_unknown_to_known(
    c, s, a, b, df_left, df_right, ddf_left_unknown, ddf_right, on, how, shuffle_method
):
    # Compute expected
    expected = df_left.merge(df_right, on=on, how=how)
    ...
    result_graph = ddf_left_unknown.merge(
        ddf_right, on=on, how=how, shuffle=shuffle_method
    )
    if not any(
        isinstance(layer, (HashJoinP2PLayer, P2PShuffleLayer))
        for layer in result_graph.dask.layers.values()
